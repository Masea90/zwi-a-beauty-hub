-- 1) Fix mutable search_path on the email queue helper functions
...
-- 2) These SECURITY DEFINER functions are only used server-side (edge functions / cron).
-- Remove public and end-user EXECUTE access.
...
-- 3) Internal helpers that are never called directly from the client
